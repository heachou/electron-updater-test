...
export function initSmallestUpdater(mainWindow: BrowserWindow): SmallestUpdater {
  ...
  const smallestUpdater = new SmallestUpdater({
    logger,
    publish: {
      url: 'http://192.168.1.10:3020/smallest-updates'
    },
    autoDownload: false,
  ...
  smallestUpdater.on('update-available', (info) => {
    ...
    dialog
      .showMessageBox(mainWindow, {
        title: '版本更新,smallest-updater',
        message: `发现新版本smallest-updater ${version}，是否更新\n\n${releaseNotes}`,
        type: 'info',
        buttons: ['稍后更新', '立即更新']
      })
      .then(({ response }) => {
...
export function initAutoUpdater(mainWindow: BrowserWindow): void {
  ...
  autoUpdater.on('update-available', (info) => {
    ...
    dialog
      .showMessageBox(mainWindow, {
        title: '版本更新,initAutoUpdater',
        message: `发现新版本 initAutoUpdater ${version}，是否更新\n\n${releaseNotes}`,
        type: 'info',
        buttons: ['稍后更新', '立即更新']
      })
      .then(({ response }) => {
